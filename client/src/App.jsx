                        ...
                        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-8 rounded-2xl w-full max-w-2xl shadow-2xl space-y-6 max-h-[90vh] overflow-auto custom-scrollbar">
                            <div className="flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 text-sm">Define las franjas horarias de atención.</p>
                                </div>
                            ...
                            <form onSubmit={handleSaveSchedule} className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700/50 space-y-6">
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                        <label className="text-xs font-bold text-slate-500 dark:text-slate-500 uppercase">Días de Atención</label>
                                        <div className="flex gap-2">
                                            <button type="button" onClick={() => [0, 1, 2, 3, 4, 5, 6].forEach(i => { const el = document.getElementById(`day-${i}`); if (el) el.checked = true; })} className="text-[10px] bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 px-2 py-1 rounded text-slate-600 dark:text-slate-300 transition-colors">Toda la semana</button>
                                            <button type="button" onClick={() => { [0, 6].forEach(i => { const el = document.getElementById(`day-${i}`); if (el) el.checked = false; });[1, 2, 3, 4, 5].forEach(i => { const el = document.getElementById(`day-${i}`); if (el) el.checked = true; }); }} className="text-[10px] bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 px-2 py-1 rounded text-slate-600 dark:text-slate-300 transition-colors">Lunes a Viernes</button>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {['Dom', 'Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab'].map((day, i) => (
                                            <label key={i} className="flex-1 min-w-[60px] relative group cursor-pointer">
                                                <input type="checkbox" id={`day-${i}`} className="peer hidden" />
                                                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-2 rounded-lg text-center text-xs font-bold text-slate-400 peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-500 transition-all border-dashed peer-checked:border-solid hover:bg-slate-50 dark:hover:bg-slate-800">
                                                    {day}
                                                </div>
                                            </label>
                                        ))}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                    ...
                                    <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20 active:scale-95">
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {doctorSchedules.map((sch) => (
                                            <div key={sch.id} className="flex justify-between items-center bg-slate-50 dark:bg-slate-700/30 p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-all">
                                                ...
                                                <button onClick={() => handleDeleteSchedule(sch.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        ))}
